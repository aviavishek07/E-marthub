...
export class AuthService {
  ...
  addAddress(addaddress:any):Observable<any>{
    return this.http.post('http://localhost:8081/address/addaddress', addaddress);
  }

  getUserRole(mail: any){
    console.log(mail);
    return this.http.get(`http://localhost:8081/user/role/${mail}`, {responseType: 'text'});
  }
  ...
  getUserDetails(mail: any):Observable<any>{
    return this.http.get(`http://localhost:8081/user/userdetails/${mail}`);
  }

  getAddressDetails(mail: any):Observable<any>{
    return this.http.get(`http://localhost:8081/address/a/${mail}`);
  }
